...
int main(int argc, char *argv[])
{
	...
	else // others, no extra header * test *
		IntHeader::mode = IntHeader::NORMAL; // IntHeader::mode = IntHeader::NONE;
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 1){ // is switch
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++){
				...
				uint32_t headroom = rate * delay / 8 / 1000000000 * 3; // * 3
				
				/** test **/
			//	std::cout << i << " " << j << " " << headroom << " " << buffer_size * 1024 * 1024 << std::endl;
				/** test **/
				
				/** test **/
				// sw->m_mmu->ConfigHdrm(j, headroom);
			//	uint32_t headroomSWING = 120 * 1024;
				sw->m_mmu->ConfigHdrm(j, headroom);
				/** test **/
